...
# Locations referenced multiple times
config=$INSTALL_DIR/files/zlux/config
appServer=$ZOWE_ROOT_DIR/zlux-app-server
pinnedPlugins=$appServer/pluginDefaults/org.zowe.zlux.ng2desktop/ui/launchbar/plugins
allowedPlugins=$appServer/pluginDefaults/org.zowe.zlux.bootstrap/plugins/

# Extract archives (zlux-core.pax in root, others in sub-directory)
if test "$debug" && echo "for file in $list"
for file in $list
do
...
# Mark executable as program controlled (requires security permit)
_cmd extattr +p $appServer/bin/zssServer

# ---

# removed - nodeLogs in zowe.yaml sets explicit log directory
# TODO why is this not in pax?
# TODO zlux-app-server/log requires new R/W file system when ZOWE_ROOT_DIR is mounted R/O
#_cmd mkdir -p $appServer/log

# TODO why are these not in pax with files that are copied in?
_cmd mkdir -p $allowedPlugins
_cmd mkdir -p $pinnedPlugins

# TODO why are these not in pax?
# Add default config files
echo "  Copy of $config/<...> into $appServer/<...>" >> $LOG_FILE
_cmd cp -f $config/allowedPlugins.json $allowedPlugins/
_cmd cp -f $config/pinnedPlugins.json  $pinnedPlugins/
_cmd cp -f $config/zluxserver.json     $appServer/config/
_cmd cp -f $config/plugins/*           $appServer/plugins/          #*/

# Remove install source if requested
if test "$ReMoVe" 
then 
  _cmd rm -f $config/allowedPlugins.json
  _cmd rm -f $config/pinnedPlugins.json
  _cmd rm -f $config/zluxserver.json
  _cmd rm -f $config/plugins/*
fi    #
...
#_cmd chmod 777 $appServer/log
#_cmd chmod ug+w $appServer/bin/zssServer
...
# TODO moved here to not execute, but should be revisited

# TODO why is there a missing line in zluxserver.json? -> update source
if grep -q gatewayPort "$config/zluxserver.json"
then  # copy as-is
  _cmd cp -f $config/zluxserver.json $appServer/config/
else  # add missing line
  # note: _cmd chokes on blanks in line='        "gatewayPort": 10010,'
# _cmd --repl $appServer/config/zluxserver.json \
...
#   $config/zluxserver.json
  ...
  test "$debug" && echo \
    "awk -v line='        \"gatewayPort\": 10010,' \
    '/hostname/{printf(\"%s\n\",line)} {print $0}' \
    $config/zluxserver.json 2>&1 > $appServer/config/zluxserver.json"
  awk -v line='        "gatewayPort": 10010,' \
  '/hostname/{printf("%s\n",line)} {print $0}' \
  $config/zluxserver.json 2>&1 > $appServer/config/zluxserver.json
  sTaTuS=$?
