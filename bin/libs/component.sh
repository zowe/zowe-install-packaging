...
###############################
# Get jobname for component
#
# This function retrieves the jobname for a component by checking:
# - manifest jobnameSuffix property (combined with job prefix)
# - manifest jobname property
...
# @param string     path to component manifest file (optional)
# Output            jobname will be written to stdout
get_jobname_for_component() {
  component_name="${1}"
  component_manifest="${2}"
  
  # Check manifest for jobnameSuffix first
  if [ -n "${component_manifest}" ] && [ -f "${component_manifest}" ]; then
    jobname_suffix=$(read_yaml "${component_manifest}" ".jobnameSuffix" 2>/dev/null || true)
    if [ -n "${jobname_suffix}" ]; then
      # Get job prefix from config
      job_prefix=$(read_yaml "${ZWE_CLI_PARAMETER_CONFIG}" ".zowe.job.prefix" 2>/dev/null || true)
      echo "${job_prefix}${jobname_suffix}"
      return 0
    fi
    
    # Check for jobname directly in manifest
    jobname=$(read_yaml "${component_manifest}" ".jobname" 2>/dev/null || true)
    if [ -n "${jobname}" ]; then
      echo "${jobname}"
      return 0
    fi
  ...
  case "${component_name}" in
    gateway)
      ...
      ;;
    discovery)
      # Check if apiml is using modulith
      apiml_enabled=$(read_yaml "${ZWE_CLI_PARAMETER_CONFIG}" ".components.apiml.enabled" 2>/dev/null || true)
    ...
    zss)
      ...
      ;;
    app-server)
      nlux_no_cluster=$(read_yaml "${ZWE_CLI_PARAMETER_CONFIG}" ".zowe.environments.ZLUX_NO_CLUSTER" 2>/dev/null || true)
      if [ "${nlux_no_cluster}" = "1" ] || [ "${ZLUX_NO_CLUSTER}" = "1" ]; then
        echo "${job_prefix}DS"
      else
