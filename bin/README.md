...
Zowe provides `zs` command can help you manage Zowe instance.
...
```
# enable Zowe zs command
export PATH=${PATH}:/path/to/my/zowe/bin
```

Now you can issue `zs` command in your USS terminal. You can start with `zs --help` to see what commands you can use.
...
- `.parameters`: This is an _optional_ definition of command line parameters for this command. Parameters defined in this file will be combined with all upper level `.parameters` file and be available for this command. `zs` command has global parameters like `--help`, `--verbose`, etc, which are defined in `bin/commands/.parameters` file. Every line of this file represents a CLI parameter. Each parameter definition is separated by `|` into 5 fields:
  1. is the parameter id, or full name. This column is required and the user can use `--` prefix to customize this parameter. For example, with `config` definition, the user can use `--config` to pass value to `zs` command.
  2. is the parameter alias. This column is optional. This column is usually with one letter and the user can use `-` prefix to customize this parameter. For example, with `c` definition, the user can use `-c` to pass value to `zs` command.
...
`zs` command may exit with non-zero code with errors.
