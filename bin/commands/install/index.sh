...
if [ -n "${ZWE_CLI_PARAMETER_DATASET_PREFIX+foo}" ]; then
  _CEE_RUNOPTS="${CEE_RO}" ${ZWE_zowe_runtimeDirectory}/bin/utils/configmgr -script "${ZWE_zowe_runtimeDirectory}/bin/commands/install/cliPrefix.js"
else
  ...
  if [ -n "${ZWE_CLI_PARAMETER_CONFIG}" ]; then
    _CEE_RUNOPTS="${CEE_RO}" ${ZWE_zowe_runtimeDirectory}/bin/utils/configmgr -script "${ZWE_zowe_runtimeDirectory}/bin/commands/install/cliYaml.js"
  else
