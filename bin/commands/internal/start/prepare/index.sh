      ...
      if [ -n "${component_dir}" ]; then
        ...
        if [ -n "${configure_script}" ]; then
          if [ -f "${configure_script}" ]; then
            ...
            result=$(
              load_environment_variables "${component_id}" && . ${configure_script}
              rc=$?
              get_environment_exports >"${ZWE_PRIVATE_WORKSPACE_ENV_DIR}/${component_name}/.${ZWE_CLI_PARAMETER_HA_INSTANCE}.env"
              return $rc
            )
            retval=$?
