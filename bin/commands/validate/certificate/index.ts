...
function processCertificateAnalyserNonZeroOutput(
  output: string,
  configLines: string[],
  zoweConfig: any,
  keystoreType: string,
  ...
  keyringUser: string | undefined,
  verifyCertificates: string | undefined,
): { certificateInvalid: number; configInvalid: number } {
  ...
  if (output.includes('unknown protocol: ')) {
    ...
    if (matchesTruststore) {
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `Verify support for Zowe YAML zowe.certificate.truststore.type: ${zoweConfig.zowe.certificate.truststore.type}`);
    }
    if (matchesKeystore) {
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `Verify support for Zowe YAML zowe.certificate.keystore.type: ${zoweConfig.zowe.certificate.keystore.type}`);
    }
  }
  ...
  if (output.includes('no such provider:') ||
    ...
    common.printFormattedWarn(common.MSG_KEY, COMMAND_NAME, `  java.home: ${zoweConfig.java.home}`);
    common.printFormattedWarn(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.keystore.type: ${zoweConfig.zowe.certificate.keystore.type}`);
    common.printFormattedWarn(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.truststore.type: ${zoweConfig.zowe.certificate.truststore.type}`);
  }
  ...
  if (output.includes('Invalid keystore format')) {
    ...
    common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `Review Zowe YAML properties:`);
    common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.keystore.type: ${zoweConfig.zowe.certificate.keystore.type}`);
    common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.truststore.type: ${zoweConfig.zowe.certificate.truststore.type}`);
  }
  ...
  if (output.includes("The certificate is expired")) {
    ...
    common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `Certificate ${keystoreAlias} in keystore ${zoweConfig.zowe.certificate.keystore.file} has expired.`);
    try {
  ...
  if (output.includes("Not matched hostnames with the certificate:")) {
    ...
    if (verifyCertificates != 'STRICT') {
      ...
      if (zoweConfig.zowe.externalDomains.length > certificateCount) {
        common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `C) Remove the unmatched hostnames from zowe.externalDomains`);
      }
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `Review Zowe YAML properties:`);
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.keystore.file: ${zoweConfig.zowe.certificate.keystore.file}`);
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `  zowe.certificate.keystore.alias: ${zoweConfig.zowe.certificate.keystore.alias}`);
      common.printFormattedError(common.MSG_KEY, COMMAND_NAME, `  zowe.externalDomains: ${zoweConfig.zowe.externalDomains.join(',')}`);
    }
  }
  ...
  return { certificateInvalid, configInvalid };
}
...
export function execute(quitOnError?: boolean, level?: string): number {
  ...
  if (rc == 0) {
    ...
    const failureResult = processCertificateAnalyserNonZeroOutput(
      output,
      configLines,
      ZOWE_CONFIG,
      keystoreType,
    ...
    certificateInvalid = failureResult.certificateInvalid;
    configInvalid = failureResult.configInvalid;
  }
...
export const _unit_test = {
  processCertificateAnalyserNonZeroOutput,
  VALIDATION_OK,
  VALIDATION_WARN,
  VALIDATION_ERROR,
}
