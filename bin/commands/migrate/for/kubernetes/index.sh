...
if [ "${ZWE_zowe_verifyCertificates}" = "DISABLED" ]; then
  print_message "SECURITY WARNING: It's not recommended to DISABLE zowe.verifyCertificates for security"
  print_message "                  reasons."
  print_message
fi
...
if [ "${ZWE_zowe_certificate_keystore_type}" = "JCERACFKS" ]; then
  ...
  keyring_export_all_to_pkcs12 "${keyring_owner}" "${keyring_name}" "${temp_dir}" "${ZWE_CLI_PARAMETER_PASSWORD}"
  print_message
  exit 1
fi

if [ "$(is_certificate_generated_by_zowe)" != "true" ]; then
  print_message "It seems you are using certificates NOT generated by Zowe."
  print_message

  if [ "${ZWE_zowe_verifyCertificates}" = "STRICT" ]; then
    print_message "To make certificates working in Kubernetes, the certificate you are using should have"
    print_message "these domains defined in Subject Alt Name (SAN):"
    print_message
    for host in $(echo "${ZWE_CLI_PARAMETER_DOMAINS}" | sed 's#[,]# #g'); do
      print_message "- ${host}"
    done
    print_message "- *.${ZWE_CLI_PARAMETER_K8S_NAMESPACE}.svc.${ZWE_CLI_PARAMETER_K8S_CLUSTER_NAME}"
    print_message "- *.discovery-service.${ZWE_CLI_PARAMETER_K8S_NAMESPACE}.svc.${ZWE_CLI_PARAMETER_K8S_CLUSTER_NAME}"
    print_message "- *.gateway-service.${ZWE_CLI_PARAMETER_K8S_NAMESPACE}.svc.${ZWE_CLI_PARAMETER_K8S_CLUSTER_NAME}"
    print_message "- *.${ZWE_CLI_PARAMETER_K8S_NAMESPACE}.pod.${ZWE_CLI_PARAMETER_K8S_CLUSTER_NAME}"
    print_message
    print_message "Otherwise you may see warnings/errors related to certificate validation."
    print_message
    print_message "If you cannot add those domains into certificate Subject Alt Name (SAN), you can change"
    print_message "zowe.verifyCertificates to NONSTRICT. Zowe components will not validate domain names but"
    print_message "will continue to validate certificate chain, validity and whether it's trusted in Zowe"
    print_message "truststore. It's not recommended to disable zowe.verifyCertificates."
    print_message
  fi
else
  print_message "It seems you are using Zowe generated certificates."
  print_message
  ...
  elif [ "${ZWE_zowe_verifyCertificates}" = "NONSTRICT" ]; then
    print_message "You are using Non-Strict verify certificate mode. You existing certificates"
    print_message "should work in Kubernetes without change."
    print_message
  fi
fi
...
update_zowe_yaml "${temp_dir}/zowe.yaml" "zowe.externalPort" "7554"
...
print_message "Please note that depending on how you choose to Zowe Kubernetes gateway service,"
print_message "you need to match \"zowe.externalPort\" to be the port you are using. For example:"
print_message
print_message "zowe:"
print_message "  externalPort: \"32554\""
print_message
