...
download_apiml_artifacts() {
  ...
  node .${SCRIPT_DIR}/utils/curl.js $path/"$version"/"$full_name" -o "${temporary_components_directory}"
  rc=$?;
...
download_other_artifacts() {
  ...
  print_and_log_message "Downloading the ${artifact_name} artifact..."
  node .${SCRIPT_DIR}/utils/curl.js "$url" -o "${temporary_components_directory}"
  rc=$?;
