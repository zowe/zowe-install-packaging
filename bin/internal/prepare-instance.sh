  ...
  do
    ...
    if [ -n "${component_dir}" ]; then
      ...
      if [ -x "${validate_script}" ]; then
        ...
        result=$(. ${INSTANCE_DIR}/bin/instance/internal/read-instance.sh -i "${ZWELS_HA_INSTANCE_ID}" -o "${component_id}" && . ${validate_script})
        retval=$?
        if [ -n "${result}" ]; then
          if [ "${retval}" = "0" ]; then
  ...
  do
    ...
    if [ -n "${component_dir}" ]; then
      ...
      if [ -x "${preconfigure_script}" ]; then
        ...
        result=$(. ${INSTANCE_DIR}/bin/instance/internal/read-instance.sh -i "${ZWELS_HA_INSTANCE_ID}" -o "${component_id}" && . ${preconfigure_script} ; rc=$? ; return $rc)
        retval=$?
        if [ -n "${result}" ]; then
          if [ "${retval}" = "0" ]; then
