...
# define global tls ciphers
# >>> node.js
ZWE_DEFAULT_TLS_CIPHERS_NODE=ECDHE-RSA-AES256-GCM-SHA384,ECDHE-RSA-AES128-GCM-SHA256,ECDHE-RSA-AES128-SHA256,ECDHE-ECDSA-AES128-GCM-SHA256,ECDHE-ECDSA-AES256-GCM-SHA384,ECDHE-ECDSA-AES128-SHA256,ECDHE-ECDSA-AES256-SHA384
if [ -z "${NODE_OPTIONS}" ]; then
  NODE_OPTIONS="--tls-cipher-list=${ZWE_DEFAULT_TLS_CIPHERS_NODE}"
elif [[ ${NODE_OPTIONS} != *"--tls-cipher-list="* ]]; then
  NODE_OPTIONS="${NODE_OPTIONS} --tls-cipher-list=${ZWE_DEFAULT_TLS_CIPHERS_NODE}"
fi
# >>> java
ZWE_DEFAULT_TLS_CIPHERS_JAVA=TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256,TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256,TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA384
if [ -z "${IBM_JAVA_OPTIONS}" ]; then
  IBM_JAVA_OPTIONS="-Dserver.ssl.ciphers=${ZWE_DEFAULT_TLS_CIPHERS_JAVA}"
elif [[ ${IBM_JAVA_OPTIONS} != *"-Dserver.ssl.ciphers="* ]]; then
  IBM_JAVA_OPTIONS="${IBM_JAVA_OPTIONS} -Dserver.ssl.ciphers=${ZWE_DEFAULT_TLS_CIPHERS_JAVA}"
fi
if [ -z "${JAVA_TOOL_OPTIONS}" ]; then
  JAVA_TOOL_OPTIONS="-Dserver.ssl.ciphers=${ZWE_DEFAULT_TLS_CIPHERS_JAVA}"
elif [[ ${JAVA_TOOL_OPTIONS} != *"-Dserver.ssl.ciphers="* ]]; then
  JAVA_TOOL_OPTIONS="${JAVA_TOOL_OPTIONS} -Dserver.ssl.ciphers=${ZWE_DEFAULT_TLS_CIPHERS_JAVA}"
fi
