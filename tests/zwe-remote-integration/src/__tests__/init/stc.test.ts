...
describe(`${testSuiteName}`, () => {
  ...
  describe('(SHORT)', () => {
    it('command accepts --jcl flag', async () => {
      ...
      const result = await testRunner.runZweTest(cfgYaml, 'init stc --dry-run --jcl');
      expect(result.stdout).not.toBeNull();
      expect(result.cleanedStdout).toMatchSnapshot();
      expect(result.rc).toBe(0);
    });
    ...
    it('command errors out on configmgr with no jcl', async () => {
      cfgYaml.zowe.setup.jcl.enable = false;
      const result = await testRunner.runZweTest(cfgYaml, 'init stc --dry-run -c "FILE(zowe.test.yaml)"');
      expect(result.stdout).not.toBeNull();
      expect(result.cleanedStdout).toMatchSnapshot();
