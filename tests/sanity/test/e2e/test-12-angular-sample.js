...
const debug = require('debug')('zowe-sanity-test:e2e:angular-sample');
...
describe(`test ${APP_TO_TEST}`, function() {
  ...
  it('should launch app correctly', async function() {
    ...
    const canvas = await waitUntilElement(driver, 'app-root', viewport);
  ...
  it(`should be able to launch "${APP_NAME_TO_LAUNCH}" app`, async function() {
    ...
    // check app id
    const appNameInput = await getElement(driver, 'div.div-input input');
    ...
    const appButton = await getElement(driver, 'div.bottom-10 button.zlux-button');
