...
const debug = require('debug')('zowe-sanity-test:e2e:login');
...
describe('test MVD login page', function() {
  ...
  before('verify environment variable and load login page', async function() {
    ...
    await driver.get(`https://${process.env.ZOWE_EXTERNAL_HOST}:${process.env.ZOWE_API_MEDIATION_GATEWAY_HTTP_PORT}/zlux/ui/v1/ZLUX/plugins/org.zowe.zlux.bootstrap/web/`);
    try {
      await driver.wait(
        async() => {
