...
then
    ...
    printAndLog "  ZOWE_ROOT_DIR not specified:  Defaulting to ~/zowe/$ZOWE_VERSION"
fi
if [[ $ZOWE_PREFIX == "" ]]
then
    ZOWE_PREFIX="ZOWE"
    printAndLog "  ZOWE_PREFIX not specified:  Defaulting to ZOWE"
fi
if [[ $ZOWE_INSTANCE == "" ]]
then
    ZOWE_INSTANCE="1"
    printAndLog "  ZOWE_INSTANCE not specified:  Defaulting to 1"
fi
if [[ $ZOWE_EXPLORER_SERVER_JOBS_PORT == "" ]]
then
    ZOWE_EXPLORER_SERVER_JOBS_PORT=7080
    printAndLog "  ZOWE_EXPLORER_SERVER_JOBS_PORT not specified:  Defaulting to 7080"
fi
if [[ $ZOWE_EXPLORER_SERVER_DATASETS_PORT == "" ]]
then
    ZOWE_EXPLORER_SERVER_DATASETS_PORT=8547
    printAndLog "  ZOWE_EXPLORER_SERVER_DATASETS_PORT not specified:  Defaulting to 8547"
fi
if [[ $ZOWE_ZLUX_SERVER_HTTPS_PORT == "" ]]
then
    ZOWE_ZLUX_SERVER_HTTPS_PORT=8544
    printAndLog "  ZOWE_ZLUX_SERVER_HTTPS_PORT not specified:  Defaulting to 8544"
fi
if [[ $ZOWE_ZSS_SERVER_PORT == "" ]]
then
    ZOWE_ZSS_SERVER_PORT=8542
    printAndLog "  ZOWE_ZSS_SERVER_PORT not specified:  Defaulting to 8542"
fi
if [[ $ZOWE_ZSS_XMEM_SERVER_NAME == "" ]]
then
    ZOWE_ZSS_XMEM_SERVER_NAME=ZWESIS_STD
    printAndLog "  ZOWE_ZSS_XMEM_SERVER_NAME not specified:  Defaulting to ZWESIS_STD"
fi
if [[ $ZOWE_EXPLORER_JES_UI_PORT == "" ]]
then
    ZOWE_EXPLORER_JES_UI_PORT=8546
    printAndLog "  ZOWE_EXPLORER_JES_UI_PORT not specified:  Defaulting to 8546"
fi
if [[ $ZOWE_EXPLORER_MVS_UI_PORT == "" ]]
then
    ZOWE_EXPLORER_MVS_UI_PORT=8548
    printAndLog "  ZOWE_EXPLORER_MVS_UI_PORT not specified:  Defaulting to 8548"
fi
if [[ $ZOWE_EXPLORER_USS_UI_PORT == "" ]]
then
    ZOWE_EXPLORER_USS_UI_PORT=8550
    printAndLog "  ZOWE_EXPLORER_USS_UI_PORT not specified:  Defaulting to 8550"
fi
if [[ $ZOWE_APIM_CATALOG_PORT == "" ]]
then
    ZOWE_APIM_CATALOG_PORT=7552
    printAndLog "  ZOWE_APIM_CATALOG_PORT not specified:  Defaulting to 7552"
fi
if [[ $ZOWE_APIM_DISCOVERY_PORT == "" ]]
then
    ZOWE_APIM_DISCOVERY_PORT=7553
    printAndLog "  ZOWE_APIM_DISCOVERY_PORT not specified:  Defaulting to 7553"
fi
if [[ $ZOWE_APIM_GATEWAY_PORT == "" ]]
then
    ZOWE_APIM_GATEWAY_PORT=7554
    printAndLog "  ZOWE_APIM_GATEWAY_PORT not specified:  Defaulting to 7554"
fi
if [[ $ZOWE_APIM_VERIFY_CERTIFICATES == "" ]]
then
    ZOWE_APIM_VERIFY_CERTIFICATES="true"
    printAndLog "  ZOWE_APIM_VERIFY_CERTIFICATES not specified:  Defaulting to true"
fi
if [[ $ZOWE_APIM_ENABLE_SSO == "" ]]
then
    ZOWE_APIM_ENABLE_SSO="false"
    printAndLog "  ZOWE_APIM_ENABLE_SSO not specified:  Defaulting to false"
fi
if [[ $ZOWE_ZOSMF_KEYRING == "" ]]
then
    ZOWE_ZOSMF_KEYRING="IZUKeyring.IZUDFLT"
    printAndLog "  ZOWE_ZOSMF_KEYRING not specified:  Defaulting to IZUKeyring.IZUDFLT"
fi
if [[ $ZOWE_ZOSMF_USERID == "" ]]
then
    ZOWE_ZOSMF_USERID="IZUSVR"
    printAndLog "  ZOWE_ZOSMF_USERID not specified:  Defaulting to IZUSVR"
fi
if [[ $ZOWE_ZOSMF_ADMIN_GROUP == "" ]]
then
    ZOWE_ZOSMF_ADMIN_GROUP="IZUADMIN"
    printAndLog "  ZOWE_ZOSMF_ADMIN_GROUP not specified:  Defaulting to IZUADMIN"
fi 
...
then
    ...
    printAndLog "  ZOWE_SERVER_PROCLIB_MEMBER not specified:  Defaulting to ZOWESVR"
fi
if [[ $ZOWE_SERVER_PROCLIB_DSNAME == "" ]]
then
    ZOWE_SERVER_PROCLIB_DSNAME=auto
    printAndLog "  ZOWE_SERVER_PROCLIB_DSNAME not specified:  PROCLIB DSNAME will be selected automatically"
fi
