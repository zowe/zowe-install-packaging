...
then
  # roundabout way to ensure path is cleaned
  export INSTALL_DIR=$(cd $(dirname $ReAl);pwd)
fi    #
...
# head will limit output to first match
# sample output:
# 1.0.0
export ZOWE_VERSION=$(sed -n '/"version"/s/.*: "\([^"]*\).*/\1/p' \
  $INSTALL_DIR/manifest.json | head -1)
