...
if test "$pdse" -a "$pds"
then
  echo "** ERROR $me faulty startup argument: -e mutually exclusive with -p"
  echo "-e -p $dsn $recFm $lRecL $dsOrg"
  rc=8
fi    #

if test "$pdse" -a "$dsOrg" != "PO"
then
  echo "** ERROR $me faulty startup argument: -e requires PO, not $dsOrg"
  echo "-e $dsn $recFm $lRecL $dsOrg"
  rc=8
fi    #

if test "$pds" -a "$dsOrg" != "PO"
then
  echo "** ERROR $me faulty startup argument: -p requires PO, not $dsOrg"
  echo "-p $dsn $recFm $lRecL $dsOrg"
  rc=8
fi    #
