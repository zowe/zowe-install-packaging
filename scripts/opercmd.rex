 ...
 */
_sdsfConsoleCmd: PROCEDURE EXPOSE ExecName Debug
 ...
 */
_tsoConsoleON: PROCEDURE EXPOSE ExecName Debug SolMsg
parse arg Name
...
if cRsn == 36              /* User does not have CONSOLE authority ? */
then cRC=1                   
else if cRsn > 0
  then do  /* try 1 failed */
    MaxTries=5
    for T=2 to MaxTries do
...
then do
  /* show current values */
  if Debug then call _tsoCmd "CONSPROF"     /* ignore possible error */
  
  /* save current values */
  SolMsg='SOLDISPLAY('sysvar('SOLDISP')') SOLNUM('sysvar('SOLNUM')')'
  ...
  parse value MaxSolNum sysvar('SOLNUM') with MaxSolNum . /* current */
                                                 /* value is default */
 ...
 */
_tsoConsoleCmd: PROCEDURE EXPOSE ExecName Debug
parse arg Cmd,Delay
...
then cRC=1
  ...
  then do
    ...
    then do T=1 to ISFULOG.0; say ISFULOG.T; end
      ...
      cRC=2
      if Debug then do T=1 to ISFULOG.0; say '.' ISFULOG.T; end
    end    /* */
  end    /* */
end    /* */
 ...
 * assumes caller did outtrap('Line.','*','NOCONCAT')
 */
_tsoConsoleOFF: PROCEDURE EXPOSE ExecName Debug SolMsg
 ...
 * assumes caller did outtrap('Line.','*','NOCONCAT')
 */
_apf: PROCEDURE EXPOSE ExecName Debug Line.
parse upper Dsn
...
then do 
  ...
  then cRC=-2
    ...
    DSCB=''
    do T=T+1 to Line.0
    call value 'Line.'line.0+1,'--'          /* add stopper */
    DSCB=''
    T=T+1
 ...
 */
_tsoCmd: PROCEDURE EXPOSE ExecName Debug Line.
parse arg Cmd
if Debug then say '> _tsoCmd' Cmd

address TSO "Cmd"            /* "address TSO" for usage in z/OS UNIX */
if Debug then
  say '. "'Cmd'" ended with RC' rc
  do T=1 to Line.0; say '.' Line.T; end
end    /* */

if Debug then say '< _tsoCmd' rc
return rc    /* _tsoCmd */
...
return value('_txt.'arg(1))    /* _isfcallText */
