...
echo "Updating ports in zluxserver.json "$ZOWE_ZLUX_SERVER_HTTPS_PORT";"$ZOWE_ZLUX_SERVER_HTTP_PORT";"$ZOWE_APIM_DISCOVERY_PORT";"$ZOWE_ZSS_SERVER_PORT  >> $LOG_FILE 
...
sed 's/"port": 10011/"port": '"${ZOWE_APIM_DISCOVERY_PORT}"'/g' $TEMP_DIR/transform2.json > $TEMP_DIR/transform3.json
sed 's/"zssPort":8542/"zssPort": '"${ZOWE_ZSS_SERVER_PORT}"'/g' $TEMP_DIR/transform3.json > zluxserver.json
