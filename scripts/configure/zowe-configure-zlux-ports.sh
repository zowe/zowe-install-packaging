...
sed 's/"privilegedServerName": "ZWESIS_STD",/"privilegedServerName": "'"${ZOWE_ZSS_XMEM_SERVER_NAME}"'",/g' $TEMP_DIR/transform4.json > $TEMP_DIR/transform5.json
...
chmod -R u+w ../../vt-ng2/
_BPXK_AUTOCVT=ON sed 's/"port": 22,/"port": '"${ZOWE_ZLUX_SSH_PORT}",'/g' ../../vt-ng2/_defaultVT.json > $TEMP_DIR/_defaultVT.json
...
_BPXK_AUTOCVT=ON sed 's/"port": 23,/"port": '"${ZOWE_ZLUX_TELNET_PORT}",'/g' ../../tn3270-ng2/_defaultTN3270.json > $TEMP_DIR/_defaultTN3270.json
...
then
    ...
    _BPXK_AUTOCVT=ON sed 's/"type": "telnet"/"type": "'"${ZOWE_ZLUX_SECURITY_TYPE}"'"/' ../../tn3270-ng2/_defaultTN3270.json > $TEMP_DIR/_defaultTN3270.json
    mv $TEMP_DIR/_defaultTN3270.json ../../tn3270-ng2/_defaultTN3270.json
fi 
echo "</zowe-zlux-configure-ports.sh>" >> $LOG_FILE
