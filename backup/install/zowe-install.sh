...
install_buildin_components() {
  if [ "${ZWE_RUN_ON_ZOS}" = "true" ]; then
      component_list="launcher jobs-api files-api api-catalog discovery gateway caching-service apiml-common-lib explorer-jes explorer-mvs explorer-uss app-server zss"
  else
      component_list="launcher jobs-api files-api api-catalog discovery gateway caching-service apiml-common-lib explorer-jes explorer-mvs explorer-uss app-server"
  fi
