...
install_buildin_components() {
  if [ "${ZWE_RUN_ON_ZOS}" = "true" ]; then
      component_list="launcher jobs-api files-api api-catalog discovery gateway caching-service metrics-service apiml-common-lib apiml-sample-extension common-java-lib explorer-ui-server explorer-jes explorer-mvs explorer-uss app-server zss"
  else
      component_list="launcher jobs-api files-api api-catalog discovery gateway caching-service metrics-service apiml-common-lib common-java-lib apiml-sample-extension explorer-ui-server explorer-jes explorer-mvs explorer-uss app-server"
  fi
