...
function jfrog_search_latest {
    ...
    out=$(jfrog rt search --retries 10 --retry-wait-time 5s --sort-by=created --sort-order=desc --limit=1 "$search_pattern" 2> /dev/stderr | jq -r '.[].path')
    if [[ -z "$out" ]]; then
...
function jfrog_search_build {
    ...
    out=$(jfrog rt search --retries 10 --retry-wait-time 5s --build="$build_name/$bld_num" "$search_pattern" 2> /dev/stderr | jq -r '.[-1].path')
    if [[ "$out" == "null" ]]; then
