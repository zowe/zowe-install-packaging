...
- A data line must have this format
  <apar>
  1) <apar> List of 1 or more linked APAR numbers (IOxxxxx) to be used 
            for a build.
...
- A data line must be one of these formats:
  ...
  <apar>
  1) <ptf>  List of 1 or more linked PTF numbers (UOxxxxx) to be used 
            for a build
  2) -      Separator token between <ptf> and <apar>
  3) <apar> List of 1 or more linked APAR numbers (IOxxxxx) to be used
            for a build. If <ptf> is present, then <apar> must be the 
            APAR(s) that are the base for <ptf>.
...
- More types are possible, see the list of "System Reason IDs" in 
  "z/OS SMP/E Reference" > "SMP/E modification control statements" > 
  "++HOLD MCS"
